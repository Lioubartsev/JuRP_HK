...
void initPWM() {// ####Setup pwm####
  ...
  TCCR1A = _BV(COM1A0) | _BV(COM1B1) | _BV(WGM11) | _BV(WGM10); //FastPWM
  TCCR1B = _BV(WGM12) | _BV(CS11);// | _BV(WGM23); //prescale 8
  OCR1A = 39; // 50khz
  OCR1B = 0;//19;
...
void Zref_cb( const std_msgs::UInt16& cmd_msg){
  //on ros recieve interrupt
  digitalWrite(13, HIGH);
  uint16_t ref = 0;
 
  if(cmd_msg.data < 60000)
  {
    ref = cmd_msg.data;   
  }
  
  err = ref - enc_count/30; //positive direction up
  if (err < 0){
